...
public class UserProcess {
	...
	*/
	...
	private int handleExec(int fileNameAddr, int numArg, int argOffset){
		//System.out.println(fileNameAddr+"\n"+numArg+"\n"+argOffset);
		// Check fileNameVaddr
		...
		/*	System.out.println(fileName+": "+arguments.length);
		for(int i=0;i<arguments.length;i++)
			System.out.println("\targuments["+i+"] = "+arguments[i]);*/
		if (child.execute(fileName, arguments)){
			//System.out.println("this.id="+this.processID+"<length: "+getFileDescriptor()+">\tchild.id="+child.processID);
	...
	private int handleExit(int status){
		...
		//System.out.println("Thread ID ="+this.thread.getID()+"\n\t"+KThread.currentThread().getID());
		if (this.processID == 0) {
