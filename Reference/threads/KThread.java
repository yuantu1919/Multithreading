...
public class KThread {
	 ...
	 */
	//Yiwen
	private static ThreadQueue queueForJoin  = null;
	private static KThread t1 = null;
	...
	public static void finish() {
		...
		ThreadQueue currentQueue = currentThread.queueForJoin;

		if (currentQueue != null) {
			KThread t = currentQueue.nextThread();
			while(t != null) {
				t.ready();
				t = currentQueue.nextThread();
			}
		}
	...
	public void join() {
		...
		if (queueForJoin == null) {
			queueForJoin = ThreadedKernel.scheduler.newThreadQueue(true);
			queueForJoin.acquire(this);
		}

		if (currentThread != this && status != statusFinished) {
			queueForJoin.waitForAccess(currentThread);
			currentThread.sleep();
		}
	...
	public static void selfTest() {
		...
		KThread testThread1 = new KThread(new PingTest(1)).setName("Forkthread");
		testThread1.fork();
		PingTest pt0 = new PingTest(0);
		pt0.run();
